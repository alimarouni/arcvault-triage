...
| What did the AI get wrong? | The confidence collapse (PROMPTS.md). Also: it first targeted a Groq model that wasn't on my account, and it set `max_tokens` too low for a reasoning model so the summary call returned empty and the API reported it as a JSON error. |
